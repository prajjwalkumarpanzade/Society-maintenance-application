...
class AddBillForm(FlaskForm):
	...
	selectedWings = SelectMultipleField('Wings', choices=[],widget=ListWidget(prefix_label=False),option_widget=CheckboxInput(), validators=[Optional()])
	...
	SINKING_FUNDS		=DecimalField(label='SINKING FUNDS'       ,places=2, validators=[Optional()])
	PARKING_CHARGES		=DecimalField(label='PARKING CHARGES'     ,places=2, validators=[Optional()])
	NOC					=DecimalField(label='NOC'       ,places=2, validators=[Optional()])
	INSURANCE			=DecimalField(label='INSURANCE' ,places=2, validators=[Optional()])
